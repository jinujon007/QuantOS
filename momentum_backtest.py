...
def load_nifty50_regime(start: str, end: str) -> pd.Series:
    ...
    for ticker in ["^NSEI", "NIFTYBEES.NS"]:
        ...
        if cache_file.exists():
            ...
            if not cached.empty and cached.index.min() <= pd.Timestamp(fetch_start) and cached.index.max() >= end_tolerance:
    ...
    print(f"Regime filter active ({used_ticker}): {bear_days} bear-market days blocked out of {len(in_range)} total ({bear_days/len(in_range)*100:.1f}%)")
    return uptrend
...
def run_backtest(prices: pd.DataFrame, regime: pd.Series | None = None) -> pd.DataFrame:
    ...
    print(f"DP impact at current capital: {DP_CHARGE_PER_SCRIP/per_stock_val*100:.3f}% per sell (₹{per_stock_val:,.0f}/stock)")
    print("-" * 65)
    ...
    for step, date in enumerate(rebal_dates):
        ...
        if regime_enabled:
            ...
            if not market_up:
                ...
                port_val = cash + sum(
                    holdings[t]["shares"] * mark_prices.get(t, 0)
                    for t in holdings
                    if not pd.isna(mark_prices.get(t))
                )
...
def walk_forward_test(prices: pd.DataFrame, regime: pd.Series | None = None):
    ...
    cagr_train_filt = _cagr_of(run_backtest(p_train, regime=regime), START_DATE, END_DATE) if regime is not None else None
...
def test_regime_cache_reproducibility():
    ...
    try:
        ...
        print(f"[PASS] test_regime_cache_reproducibility (fetch calls total: {calls_after_first}, second run used cache, data identical)")
    finally:
