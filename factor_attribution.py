...
def _load_or_fetch(ticker: str, cache_path: Path, start: str, end: str) -> pd.Series:
    if cache_path.exists():
        ...
        if not cached.empty and cached.index.min() <= pd.Timestamp(start) and cached.index.max() >= pd.Timestamp(end) - pd.Timedelta(days=5):
            return cached
...
def print_report(label: str, res: dict, factor_means: dict, mean_y: float):
    ...
    for i, name in enumerate(res["names"]):
        ...
        print(f"  {name:10s}  coef={res['coefs'][i]:+.5f}  se={res['se'][i]:.5f}  "
    ...
    print(f"    alpha contribution:  {res['coefs'][0]*100:+.4f}%  ({res['coefs'][0]/mean_y*100:.1f}% of mean return)")
    ...
    print(f"    variance explained (R2): {res['r2']*100:.1f}%   variance unexplained (residual): {(1-res['r2'])*100:.1f}%")
...
def main():
    ...
    print(f"Momentum factor proxy: {MOMENTUM_FACTOR_TICKER} (UTI Nifty200 Momentum 30 Index Direct Growth), "
    ...
    print(f"Overlap-sample aligned weekly observations (momentum factor coverage): {len(df_overlap)}, "
    ...
    print_report("ONE-FACTOR MODEL, full sample 2019-2024 (market only) -- prior experiment", one_factor, factor_means_full, mean_y_full)

    two_factor_full = ols_multifactor(df_full["strat"].values, df_full[["mkt", "size"]].values, ["mkt", "size"])
    print_report("TWO-FACTOR MODEL, full sample 2019-2024 (market + size) -- prior experiment", two_factor_full, factor_means_full, mean_y_full)
    ...
    factor_means_ov = {"mkt": df_overlap["mkt"].mean(), "size": df_overlap["size"].mean(), "mom": df_overlap["mom"].mean()}

    two_factor_ov = ols_multifactor(df_overlap["strat"].values, df_overlap[["mkt", "size"]].values, ["mkt", "size"])
    print_report(f"TWO-FACTOR MODEL, OVERLAP SAMPLE ({df_overlap.index.min().date()} to {df_overlap.index.max().date()}) -- fair baseline for the 4-factor comparison below", two_factor_ov, factor_means_ov, mean_y_ov)

    four_factor = ols_multifactor(df_overlap["strat"].values, df_overlap[["mkt", "size", "mom"]].values, ["mkt", "size", "mom"])
    print_report("FOUR-FACTOR MODEL, same overlap sample (market + size + momentum)", four_factor, factor_means_ov, mean_y_ov)
    ...
    print(f"  Alpha, two-factor (mkt+size):        {alpha_2f_ov*100:+.4f}%/wk  ({(1+alpha_2f_ov)**52-1:+.2%} ann.)  p={two_factor_ov['p'][0]:.4f}")
    print(f"  Alpha, four-factor (mkt+size+mom):   {alpha_4f*100:+.4f}%/wk  ({(1+alpha_4f)**52-1:+.2%} ann.)  p={four_factor['p'][0]:.4f}")
    ...
    print(f"  R2 two-factor: {two_factor_ov['r2']:.4f}   R2 four-factor: {four_factor['r2']:.4f}   (adj-R2: {two_factor_ov['adj_r2']:.4f} -> {four_factor['adj_r2']:.4f})")
    ...
    if len(roll_alpha) > 0:
        print(f"  Rolling annualized alpha: min={roll_alpha.min()*100:+.1f}%  max={roll_alpha.max()*100:+.1f}%  mean={roll_alpha.mean()*100:+.1f}%  std={roll_alpha.std()*100:.1f}%")
